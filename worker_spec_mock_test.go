...
func (m *MockWorkerSpec) Init(v0 config.Config) error {
	r0 := m.InitFunc.nextHook()(v0)
	m.InitFunc.appendCall(WorkerSpecInitFuncCall{v0, r0})
	return r0
}
...
func (f *WorkerSpecInitFunc) SetDefaultHook(hook func(config.Config) error) {
	f.defaultHook = hook
}
...
func (f *WorkerSpecInitFunc) PushHook(hook func(config.Config) error) {
	f.mutex.Lock()
...
func (f *WorkerSpecInitFunc) nextHook() func(config.Config) error {
	f.mutex.Lock()
